...
const FRAME_MS=200,LONG_MS=550,HID_TIMEOUT_MS=Number(process.env.PI_DECK_HID_TIMEOUT_MS||5000)
export function timed(promise,ms=HID_TIMEOUT_MS){let t;const p=Promise.resolve(promise);p.catch(()=>{});const timeout=new Promise((_,reject)=>{t=setTimeout(()=>reject(new Error('hid operation timed out')),ms)});return Promise.race([p,timeout]).finally(()=>clearTimeout(t))}
...
export class Deck {
  ...
  async open(brightness=60){const [info]=await listStreamDecks();if(!info)throw new Error('no Stream Deck found');this.deck=await openStreamDeck(info.path);this.buttons=this.deck.CONTROLS.filter(c=>c.type==='button'&&c.feedbackType==='lcd');this.size=this.buttons[0]?.pixelSize||{width:80,height:80};await timed(this.deck.clearPanel());await timed(this.deck.setBrightness(Math.max(0,Math.min(100,brightness))));this.deck.on('down',c=>{if(c.type==='button')this.down.set(c.index,Date.now())});this.deck.on('up',c=>{if(c.type!=='button')return;const start=this.down.get(c.index);this.down.delete(c.index);if(start==null||this.longFired.delete(c.index))return;this.emit(c.index,Date.now()-start>=LONG_MS)});this.deck.on('error',()=>this.close());return this}
  get keyCount(){return this.buttons?.length||6}
  async serial(){try{return await timed(this.deck.getSerialNumber())}catch{return null}}
  start(){this.timer=setInterval(()=>this.tick(),FRAME_MS);this.timer.unref?.()}
  async close(){if(this.timer)clearInterval(this.timer);this.timer=null;const d=this.deck;this.deck=null;try{await timed(d?.clearPanel())}catch{}try{await timed(d?.close())}catch{}}
  ...
  async paint(i,spec,opts={}){if(!this.deck)return;if(spec.kind==='blank')return timed(this.deck.clearKey(i));await timed(this.deck.fillKeyBuffer(i,paintKey(spec,this.size,opts),{format:'rgba'}))}
  ...
  async tick(){try{this.frame++;for(const i of this.animated)await this.animate(i,this.scene[i])}catch{await this.close()}for(const [i,start] of this.down)if(!this.longFired.has(i)&&Date.now()-start>=LONG_MS){this.longFired.add(i);this.emit(i,true)}}
